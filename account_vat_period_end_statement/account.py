        ...
        for statement in self.browse(cr, uid, ids, context):
            ...
            if statement.authority_vat_amount > 0:
                ...
                if statement.payment_term_id:
                    ...
                    if len(due_list) == 0:
                        raise orm.except_orm(_('Error'),
                                             _('The payment term %s does not have due dates') % statement.payment_term_id.name)
    ...
    def compute_amounts(self, cr, uid, ids, context=None):
        ...
        for statement in self.browse(cr, uid, ids, context):
            ...
            for credit_tax_code_id in credit_tax_code_ids:
                ...
                credit_line_ids.append({'account_id': credit_tax_code.vat_statement_account_id.id,
...
class statement_debit_account_line(orm.Model):
    ...
    _columns = {'account_id': fields.many2one('account.account', 'Account', required=True),
...
class statement_generic_account_line(orm.Model):
    ...
    def on_change_vat_account_id(self, cr, uid, ids, vat_account_id=False, context=None):
        ...
        if not vat_account_id:
            return res
        res['value']['amount'] = self.pool.get('account.account').browse(cr, uid, vat_account_id, context).balance
        return res


class account_tax_code(orm.Model):
    _inherit = "account.tax.code"
    _columns = {'vat_statement_account_id': fields.many2one('account.account', "Account used for VAT statement"),
