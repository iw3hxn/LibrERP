...
class account_aged_partner_balance(osv.TransientModel):
    _name = 'account.aged.partner.balance'
    _rec_name = 'partner_id'
    _order = 'name'

    _columns = {
        'name': fields.related('partner_id', 'name', type='char', string=u'Partner', store=True),
...
class account_aged_partner_document(osv.TransientModel):
    ...
    _inherit = 'account.aged.partner.balance'
    _rec_name = 'partner_id'
...
class account_aged_trial_balance(osv.TransientModel):
    ...
    _columns = {
        ...
        'partner_line_ids': fields.one2many('account.aged.partner.balance',
                                            'aatb_id', 'Partner Aged Trail Balance',
                                            help='Partner Aged Trail Balance'),
    ...
    def _screening_invoices(self, cr, uid, ids, form, res, context=None):
        ...
        for r_var in res:
            ...
            elif form['1']['start'] <= r_var['date_due'] <= form['1']['stop']:
                r_var['days_due_91to120'] = r_var['residual']
            elif form['2']['start'] <= r_var['date_due'] <= form['2']['stop']:
                r_var['days_due_61to90'] = r_var['residual']
            elif form['3']['start'] <= r_var['date_due'] <= form['3']['stop']:
                r_var['days_due_31to60'] = r_var['residual']
            elif form['4']['start'] <= r_var['date_due'] <= form['4']['stop']:
