...
class print_prima_nota_cassa(report_sxw.rml_parse, common_report_header):
    ...
    def get_children_accounts(self, account):
        ...
        cash_bank_accounts = [journal_obj.browse(self.cr, self.uid, j).default_credit_account_id.id for j in
                              cash_bank_journals] + \
                             [journal_obj.browse(self.cr, self.uid, j).default_debit_account_id.id for j in
                              cash_bank_journals]

        ids_acc = [acc for acc in
                   self.pool.get('account.account')._get_children_and_consol(self.cr, self.uid, account.id) \
