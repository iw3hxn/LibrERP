...
class res_partner_agent(orm.Model):
    ...
    def _get_partner_agents_to_update_from_sale_agents(self, cr, uid, ids, context=None):
        ...
        if context is None:
            context = {}
        result = []
        for agent_obj_id in self.browse(cr, uid, ids, context):
            partner_agents_ids = self.pool['res.partner.agent'].search(cr, uid, [('agent_id', '=', agent_obj_id.id)])
    ...
    _columns = {
        ...
        'type': fields.related('agent_id', 'type', type="selection",
                               selection=[('external', 'External Commercial'), ('commercial', 'Internal Commercial')],
                               readonly=True,
