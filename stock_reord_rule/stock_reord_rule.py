...
class stock_warehouse_orderpoint(orm.Model):
    ...
    def _qty_orderpoint_days(self, cr, uid, ids, context=None):
        ...
        (sum( product_uos_qty )/pp.days_stats*(1+pp.forecast_gap/100) *
        pp.days_warehouse) AS quantity FROM sale_order_line sol JOIN sale_order
        ...
        GROUP BY sol.product_uom, sol.product_id, pp.days_stats,
        pp.forecast_gap, pp.days_warehouse;"""
        cr.execute(sql, (product_ids,))
        sql_res = cr.fetchall()
        if sql_res:
            for val in sql_res:
                if val:
                    reord_rules_ids = self.search(
                        cr, uid, [('product_id', '=', val[0])],
                        context=context)
                    if reord_rules_ids:
                        self.write(
                            cr, uid, reord_rules_ids,
                            {'product_max_qty': val[1]}, context=context)
        return True
...
class product_product(orm.Model):
    ...
    _columns = {
        ...
        'forecast_gap': fields.float(
            'Expected sales variation (percent +/-)', digits=(6, 3)),
        }
