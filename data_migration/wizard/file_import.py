...
class filedata_import(orm.TransientModel):
    ...
    def actionCheckEncoding(self, cr, uid, ids, context):
        ...
        if not contentBase64:
            ...
            raise orm.except_orm('Attenzione!', 'Non è stato selezionato il file da importare')
...
class picking_import(filedata_import):
    ...
    _columns = {
        ...
        'stock_journal_id': fields.many2one('stock.journal', 'Stock Journal', select=True, required=True),
        'location_id': fields.many2one('stock.location', 'Location', select=True, required=True), # TODO add domain
        'location_dest_id': fields.many2one('stock.location', 'Dest. Location', select=True, required=True),
