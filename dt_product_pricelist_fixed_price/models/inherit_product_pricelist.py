...
class product_pricelist(orm.Model):
    ...
    def price_get_multi(self, cr, uid, pricelist_ids, products_by_qty_by_partner, context=None):
        ...
        if pricelist_ids:
            ...
            # all pricelists:
            pricelist_version_ids = self.pool['product.pricelist'].search(cr, uid, [], context=context)
        ...
        plversion_ids = product_pricelist_version_obj.search(cr, uid, plversions_search_args, context=context)
        ...
        product_categories = product_category_obj.read(cr, uid, product_category_ids, ['parent_id'], context=context)
            ...
            for pricelist_id in pricelist_version_ids:
                    ...
                    if res:
                        if res['base'] == -1:
                            if not res['base_pricelist_id']:
                                ...
                                ptype_src = self.browse(cr, uid, res['base_pricelist_id'], context=context).currency_id.id
                            ...
                            if partner:
                                where = [('name', '=', partner)]
                            sinfo = supplierinfo_obj.search(cr, uid, [('product_id', '=', tmpl_id)] + where, context=context)
                            price = 0.0
                            if sinfo:
                                qty_in_product_uom = qty
                                product_default_uom = product_obj.read(cr, uid, [tmpl_id], ['uom_id'], context=context)[0]['uom_id'][0]
                                seller_uom = supplierinfo_obj.read(cr, uid, sinfo, ['product_uom'], context=context)[0]['product_uom'][0]
                        ...
                        # End
                            ...
                            price_type = price_type_obj.browse(cr, uid, int(res['base']), context=context)
                ...
                if price:
                    if 'uom' in context and not uom_price_already_computed:
                        ...
                        price = self.pool['product.uom']._compute_price(cr, uid, uom.id, price, context['uom'])
